<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.kechengziliao" label="课程资料" prop="kechengziliao">
          <file-upload
          tip="点击上传课程资料"
          action="file/upload"
          :limit="1"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.kechengziliao" label="课程资料" prop="kechengziliao">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.kechengziliao)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      this.$http({
		...
		if(json.jiaoshiyouxiang!=''&&json.jiaoshiyouxiang){
                this.ruleForm.jiaoshiyouxiang = json.jiaoshiyouxiang
	    		this.ro.jiaoshiyouxiang = true;
		}
        } else {
	...
	this.ruleForm.kechengxiangqing = this.ruleForm.kechengxiangqing.replace(reg,'../../../djangopm60z/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.kechengziliao!=null) {
		this.ruleForm.kechengziliao = this.ruleForm.kechengziliao.replace(new RegExp(this.$base.url,"g"),"");
	}
