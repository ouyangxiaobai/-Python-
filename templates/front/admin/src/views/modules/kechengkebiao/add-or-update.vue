<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.kebiao" label="课表" prop="kebiao">
          <file-upload
          tip="点击上传课表"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.kebiao?ruleForm.kebiao:''"
          @change="kebiaoUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.kebiao" label="课表" prop="kebiao">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.kebiao.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.xiangqing = this.ruleForm.xiangqing.replace(reg,'../../../djangopm60z/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.kebiao!=null) {
		this.ruleForm.kebiao = this.ruleForm.kebiao.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `kechengkebiao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.kechengkebiaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    kebiaoUploadChange(fileUrls) {
	this.ruleForm.kebiao = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
